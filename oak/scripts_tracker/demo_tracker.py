        ...
        for hand in hands:
            ...
            if is_pinching:
                ...
                if len(pos_des) < 2:
                    ...
                    vel_des = np.linalg.norm(pos_des[1][0]-pos_des[0][0])/(pos_des[1][1]-pos_des[0][1])

                cartPose = CartesianPose([float(hand_pos[0]), float(hand_pos[2]),
                                           120.0 + float(hand_pos[1]), -90.0, 0.0, 0.0])
                robot.moveToCartPose(cartPose)
                # time.sleep(1)
                print(pos_des)
                print(vel_des)
